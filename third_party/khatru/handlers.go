			...
			go func(message string) {
				...
				case *nostr.ReqEnvelope:
					...
					defer func() {
						if !listenerRegistered {
							cancelReqCtx(errors.New("request did not register a listener"))
						}
					}()
					if len(env.Filters) == 0 {
						cancelReqCtx(errors.New("request has no filters"))
						ws.WriteJSON(nostr.ClosedEnvelope{SubscriptionID: env.SubscriptionID, Reason: "invalid: request has no filters"})
						return
					}
				...
				case *nostr.AuthEnvelope:
					...
					if pubkey, err := nip42.ValidateAuthEvent(env.Event, ws.Challenge, wsBaseUrl); err == nil {
						...
						authSnapshot := slices.Clone(ws.AuthedPublicKeys)
						ws.authSnapshot.Store(&authSnapshot)
						if rl.OnAuth != nil {
